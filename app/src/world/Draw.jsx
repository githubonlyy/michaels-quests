...
// outline (pointer-events: none) — an <svg> for the built-in pages, an <img>
// with the paper keyed out for a family page from app/coloring-pages/. The
// outline sits above the paint so the black lines stay crisp while he colors,
// and the eraser can never eat it.
//
...
const PAGE_KEY = 'michaels-quests-draw-page'
 ...
 * toss for a six-year-old. Where the names collide the file name is shown
...
export default function Draw({ onClose }) {
  ...
  // family pages only: solid lines to color inside, or faint lines to trace over
  ...
  return (
    ...
    >
      ...
      <div className="flex items-center gap-2 px-2 py-2 bg-(--t-side) border-b-4 border-(--t-side-deep) shrink-0">
        ...
        {/* Only a family page has two looks, so this button appears only for one.
            The label and the icon name what the tap DOES, not what the page
            already is — as a state readout it told him nothing about the tap. */}
        {isImagePage && (
          <IconBtn
            onClick={togglePageMode}
            label={pageMode === PAGE_MODES.lines ? 'דף שקוף' : 'קווים כהים'}
            tone={pageMode === PAGE_MODES.trace ? 'accent' : undefined}
          >
            {pageMode === PAGE_MODES.lines ? <Ghost size={28} strokeWidth={2.5} /> : <Pencil size={28} strokeWidth={2.5} />}
          </IconBtn>
        )}
      ...
      {sheet === 'templates' && (
        <Sheet title="דפי צביעה" emoji="🖍️" onClose={() => setSheet(null)}>
          ...
          {/* pictures Lior drops into app/coloring-pages/ — see the README there */}
          {hasFamilyPages && (
            <>
        ...
        <Sheet title="הציורים שלי" emoji="🖼️" onClose={() => setSheet(null)}>
          {gallery.length === 0 ? (
            <div className="flex flex-col items-center gap-3 py-10 text-center">
              ...
              <p className="text-2xl font-black text-slate-700">עוד אין ציורים — בוא נצייר!</p>
            </div>
          ) : (
...
// one picker tile, shared by the built-in pages and the family ones
