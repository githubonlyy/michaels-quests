...
describe('coloring templates', () => {
  it('has the blank page plus 8 outlines with unique ids, Hebrew names and emoji', () => {
    ...
    expect(TEMPLATES.filter((t) => t.paths.length > 0)).toHaveLength(9)
    ...
    expect(templateById('heart').id).toBe('heart')
  })
...
describe('palette', () => {
  ...
  it('the two extra colors really come from the world', () => {
    ...
    expect(extras('dinos')).toContain('#facc15')
    expect(extras('cars')[0]).toBe('#fbbf24')
    expect(extras('space')[0]).toBe('#818cf8')
