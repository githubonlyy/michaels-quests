...
git add -A
git commit -m "Deploy Michael's Quests build" --quiet
git push --force --quiet "https://github.com/githubonlyy/michaels-quests.git" gh-pages
Set-Location $root
Remove-Item -Recurse -Force "$root\app\dist\.git" -Confirm:$false

Write-Host "Deployed: https://githubonlyy.github.io/michaels-quests/" -ForegroundColor Green
